 ...
 */
public class SWBEmbedTemplates {
    private static Logger log = SWBUtils.getLogger(SWBEmbedTemplates.class);  
    ...
    public static void buildTemplate(HttpServletResponse response, VelocityContext ctx, String tplName, Resource base){
        ...
        try {
            ...
            ctx.put("webPath", SWBEmbedUtils.getWebPath(base));
    ...
    private static Template prepareTemplate(String name, Resource base){
        ...
        try {
            ...
            p.setProperty("file.resource.loader.path", SWBEmbedUtils.getWPath(base) + "templates");
            p.setProperty("file.resource.loader.cache", "false");
            ve.init(p);
            tmpl = ve.getTemplate(name, "UTF-8");                   
        } catch(Exception e){
            log.error("Ocurrió un error en el armado de la plantilla:\n "+e.getMessage());
            log.error("Path de template: " + SWBEmbedUtils.getWPath(base) + "templates");
            e.getStackTrace();
        }
        return tmpl;
    }
    
    private SWBEmbedTemplates(){}
}
